...
const InternshipDetail = () => {
  ...
  if (!internship) {
    return <div className="flex items-center justify-center h-screen text-2xl text-red-500">Internship not found</div>;
  }
      ...
      <div className="flex flex-col w-3/4 p-8 bg-white shadow-2xl rounded-2xl transform transition-all h-screen overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
        ...
        <div className="flex items-center mb-8">
          <div className="w-20 h-20 bg-gradient-to-r from-blue-500 to-blue-700 flex items-center justify-center rounded-full shadow-lg mr-6">
            <span className="text-white text-2xl font-bold">{internship.company[0]}</span>
          </div>
          <div>
            <h2 className="text-4xl font-extrabold text-gray-800">{internship.title}</h2>
            <p className="text-blue-600 text-2xl font-semibold mt-2">{internship.company}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 text-gray-900 text-lg border-t pt-6">
          <div className="flex flex-col gap-4">
            <p className="flex items-center gap-3"><FaMapMarkerAlt className="text-red-500 text-xl" /> <span className="font-bold">Location:</span> {internship.location}</p>
            <p className="flex items-center gap-3"><FaRegClock className="text-blue-500 text-xl" /> <span className="font-bold">Duration:</span> {internship.duration}</p>
          </div>
          <div className="flex flex-col gap-4">
            <p className="flex items-center gap-3"><FaRupeeSign className="text-green-600 text-xl" /> <span className="font-bold">Stipend:</span> ₹{internship.stipend}/month</p>
            <p className="flex items-center gap-3"><FaRocket className="text-purple-600 text-xl" /> <span className="font-bold">Internship Type:</span> {internship.type}</p>
          </div>
        </div>
      ...
      <div className="w-1/4 bg-white shadow-2xl rounded-2xl p-6 overflow-y-auto h-screen scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
        ...
        {internships.slice(0, 6).map((internship, index) => (
          <div key={index} className="bg-gradient-to-r from-gray-50 to-gray-100 p-6 rounded-lg shadow-lg mb-6 transition-all transform hover:scale-105 hover:shadow-xl cursor-pointer">
            <h4 className="text-xl font-semibold text-gray-800 mb-2">{internship.title}</h4>
            ...
            <p className="text-sm font-medium mb-4"><FaRupeeSign className="inline text-green-600" /> {internship.stipend} / month</p>
